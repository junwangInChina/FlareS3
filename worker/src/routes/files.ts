...
import { loadR2Config, generateDownloadUrl } from '../services/r2'
...
export async function listFiles(request: Request, env: Env): Promise<Response> {
  ...
  let whereClause = "WHERE f.upload_status IN ('completed','deleted') AND f.deleted_at IS NULL"
  ...
  const now = Date.now()
  const filesWithUrl = await Promise.all((rows.results || []).map(async row => {
    const expiresAt = new Date(String(row.expires_at)).getTime()
    const remaining = formatDuration(expiresAt - now)
    let downloadUrl = ''
    const allowDirect = Number(row.require_login) === 0
    if (allowDirect && row.upload_status === 'completed' && now < expiresAt && loaded) {
      try {
        const ttl = Math.max(60, Math.floor((expiresAt - now) / 1000))
        downloadUrl = await generateDownloadUrl(loaded.config, String(row.r2_key), String(row.filename), ttl)
      } catch (error) {
...
export async function deleteFile(request: Request, env: Env, fileId: string): Promise<Response> {
  ...
  await env.DB.prepare('UPDATE files SET upload_status = ?, deleted_at = ? WHERE id = ?')
    ...
    .run()
  await env.DB.prepare('INSERT INTO delete_queue (id, file_id, r2_key, created_at) VALUES (?, ?, ?, ?)')
